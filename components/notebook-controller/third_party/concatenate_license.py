...
def fetch_license_text(download_link):
  ...
  assert (
      response.ok
  ), f'Fetching {download_link} failed with {response.status_code} {response.reason}'
...
def main():
  with open(args.license_info_file,
    ...
    for line in license_info_file:
      ...
      try:
        print(
            f'Repo {repo} has license download link {license_download_link}',
            file=sys.stderr,
        )
        ...
        print(f'{repo}  {license_name}  {license_link}', file=output_file)
        print(
            '--------------------------------------------------------------------------------',
            file=output_file,
        )
    ...
    print(
        f'Failed to download license file for {len(repo_failed)} repos.',
        file=sys.stderr,
    )
